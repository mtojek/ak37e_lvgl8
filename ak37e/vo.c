...
void anyka_vo_init()
{
    ...
    gui_buffer = (unsigned char*) ak_mem_dma_alloc(MODULE_ID_VO, ANYKA_LCD_H * ANYKA_LCD_W * 3);
    r = ak_mem_dma_vaddr2paddr(gui_buffer, &anyka_gui_info.dma_addr);
    if (r != 0) {
	    anyka_perror("ak_mem_dma_vaddr2paddr failed", r);
	    goto ANYKA_VO_INIT_FAILED_DESTROY_GUI_LAYER;
    }

    printf("anyka_vo_init stop\n");
